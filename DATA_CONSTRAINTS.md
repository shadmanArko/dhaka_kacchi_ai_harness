...
| Warehouse ingest job reading `dhaka-kacchi-connect`'s `events` table (`warehouse/ingest/events.py`, `raw_events_direct`) | Real | Built, lint/typecheck clean, and exercised end-to-end locally (2026-09-21) against real inserted/deleted test rows: idempotent (`make verify-ingest-events`), correctly picks up only genuinely new rows on re-run, and correctly leaves `event.customer_id`/`order_id` NULL while preserving the source's own ids under `properties.source_customer_id`/`source_order_id` (identity resolution is out of scope — see the module's own docstring). Not yet run against production — the production `ordering_reader`/warehouse connection needs `make ingest-events` scheduled the same way `make ingest-direct` is (see `deploy/CLAUDE.md`'s cron example). |
